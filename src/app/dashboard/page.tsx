...
import { BookOpen, Award, CheckCircle2, Clock, Play, ArrowLeft, LogOut, ChevronRight, Globe, Sun, Moon, Star, Compass, User, Lock, Plus, Save, Trash2, Edit } from 'lucide-react';
...
export default function StudentDashboard() {
  ...
  const [courseTitleFr, setCourseTitleFr] = useState('');
  const [courseTitleEn, setCourseTitleEn] = useState('');
  const [courseDescFr, setCourseDescFr] = useState('');
  const [courseDescEn, setCourseDescEn] = useState('');
  const [courseDifficulty, setCourseDifficulty] = useState('Beginner');
  const [courseCategory, setCourseCategory] = useState('front');
  const [courseImageUrl, setCourseImageUrl] = useState('/nextjs_react_cover.png');
  const [builderModules, setBuilderModules] = useState<any[]>([]);
  const [builderLoading, setBuilderLoading] = useState(false);
  const [builderError, setBuilderError] = useState('');

  const loadReviewsAndCourses = async () => {
    try {
  ...
  const handleAddModule = () => {
    const newModId = builderModules.length + 1;
    setBuilderModules([
      ...builderModules,
      {
        id: newModId,
        titleFr: `Module ${newModId} : Nouveau Module`,
        titleEn: `Module ${newModId}: New Module`,
        ...
        lessons: [
          {
            ...
            duration: '15 min',
            contentFr: '# Introduction \n Écrivez votre leçon en français.',
            contentEn: '# Introduction \n Write your lesson in English.',
            exercise: {
              questionFr: 'Écrivez une fonction additionner(a, b) qui retourne la somme.',
              questionEn: 'Write a function additionner(a, b) that returns the sum.',
              lang: 'js',
              initialCode: 'function additionner(a, b) {\n  // Votre code ici\n}',
              testCases: [
  ...
  const handleRemoveModule = (modId: number) => {
    setBuilderModules(builderModules.filter((m) => m.id !== modId).map((m, index) => ({
      ...m,
      id: index + 1
    })));
  };
  ...
  const handleSaveCourse = async () => {
    if (!courseTitleFr || !courseTitleEn || builderModules.length === 0) {
      setBuilderError('Veuillez remplir le titre et ajouter au moins un module.');
      return;
    }
    ...
    try {
      const coursePayload = {
        id: `custom-${Date.now()}`,
        titleFr: courseTitleFr,
        titleEn: courseTitleEn,
        descriptionFr: courseDescFr,
        descriptionEn: courseDescEn,
        author: user.name,
        difficultyFr: courseDifficulty === 'Beginner' ? 'Débutant' : courseDifficulty === 'Intermediate' ? 'Intermédiaire' : 'Avancé',
        difficultyEn: courseDifficulty,
      ...
      const response = await fetch('/api/course/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: user.email, course: coursePayload })
      });
      ...
      setBuilderError(err.message || 'Error occurred');
    } finally {
  ...
  const t = {
    fr: {
      ...
      builderTitle: "Créer un nouveau programme de formation",
      builderSub: "Concevez le syllabus, les modules et les tests du playground.",
      builderBtn: "Ouvrir le Constructeur de Cours",
      builderSubmit: "Publier le cours",
      builderAddMod: "Ajouter un Module",
      builderTitleFrLabel: "Titre du Cursus (FR)",
      builderTitleEnLabel: "Titre du Cursus (EN)",
      builderDescFrLabel: "Description (FR)",
      builderDescEnLabel: "Description (EN)"
    },
    en: {
      welcome: "Account:",
      logout: "Sign Out",
      ...
      builderTitle: "Design New Learning Program",
      builderSub: "Build syllabus modules, lessons, and playground validation cases.",
      builderBtn: "Open Course Builder Panel",
      builderSubmit: "Publish Course",
      builderAddMod: "Add Module",
      builderTitleFrLabel: "Course Title (FR)",
      builderTitleEnLabel: "Course Title (EN)",
      builderDescFrLabel: "Description (FR)",
      builderDescEnLabel: "Description (EN)"
    }
  }[lang];
  ...
  const activeCourse = coursesList.find((c) => c.id === currentSelectedCourseId) || null;
  ...
  const totalLessons = activeCourse
    ? activeCourse.modules.reduce((sum: number, mod: any) => sum + mod.lessons.length, 0)
    : 0;
  ...
  const allCourseLessons = activeCourse ? activeCourse.modules.flatMap((m: any) => m.lessons) : [];

  // Check if a lesson is unlocked (first is unlocked, or previous must be completed)
  const isLessonUnlocked = (lessonId: string) => {
    if (!courseProgressState) return false;
    const idx = allCourseLessons.findIndex((l: any) => l.id === lessonId);
  ...
  const isExamUnlocked = allCourseLessons.length > 0 && allCourseLessons.every((l: any) => courseProgressState?.progress[l.id] === true);

  const getCourseRatingInfo = (courseId: string) => {
    const courseReviews = reviews.filter((r: any) => r.courseId === courseId);
    if (courseReviews.length === 0) {
      return { avg: 5, count: 0 };
    }
    const sum = courseReviews.reduce((acc: number, r: any) => acc + r.rating, 0);
    return {
      ...
      <header className="sticky top-0 z-40 glass-panel mx-4 mt-4 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <span className="text-sm font-semibold hidden md:inline">
            {t.welcome} <span className="text-blue-500">{user.name} ({user.role.toUpperCase()})</span>
          </span>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {/* Instructor Course Builder Toggle Banner */}
            {isInstructorOrAdmin && (
              <section className="p-6 rounded-2xl border border-blue-500/30 bg-blue-600/10 flex items-center justify-between gap-4">
                <div>
                  <h2 className="text-base font-bold text-blue-400 uppercase tracking-wider">{t.builderTitle}</h2>
                  <p className="text-xs text-[var(--text-secondary)] mt-1">{t.builderSub}</p>
                </div>
                <button
                  onClick={() => setIsBuildingCourse(!isBuildingCourse)}
                  className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-all shadow-md"
                >
                  {isBuildingCourse ? 'Fermer le Constructeur' : t.builderBtn}
                </button>
              </section>
            )}

            {/* Course Builder View */}
            {isBuildingCourse && isInstructorOrAdmin ? (
              <section className="glass-panel p-6 border-[var(--border)] bg-[var(--bg-secondary)]/50 space-y-6">
                <h3 className="text-lg font-bold border-b border-[var(--border)] pb-2 uppercase tracking-wide text-blue-500">
                  Course Constructor
                </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase mb-1">{t.builderTitleFrLabel}</label>
                    <input
                      type="text"
                      value={courseTitleFr}
                      onChange={(e) => setCourseTitleFr(e.target.value)}
                      className="w-full premium-input text-xs"
                      placeholder="e.g. Introduction à Next.js"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase mb-1">{t.builderTitleEnLabel}</label>
                    <input
                      type="text"
                      value={courseTitleEn}
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase mb-1">{t.builderDescFrLabel}</label>
                    <input
                      type="text"
                      value={courseDescFr}
                      onChange={(e) => setCourseDescFr(e.target.value)}
                      className="w-full premium-input text-xs"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase mb-1">{t.builderDescEnLabel}</label>
                    <input
                      type="text"
                      value={courseDescEn}
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase mb-1">Difficulté / Difficulty</label>
                    <select
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase mb-1">Catégorie / Subject</label>
                    <select
                    ...
                    >
                      ...
                      <option value="oop">Java OOP</option>
                      <option value="python">Python</option>
                    </select>
                  </div>
                </div>

                {/* Modules list creator */}
                <div className="space-y-4 pt-4 border-t border-[var(--border)]">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-slate-200">Syllabus Modules</h4>
                    <button
                      type="button"
                      onClick={handleAddModule}
                      className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-[10px] font-bold flex items-center gap-1"
                    >
                  ...
                  <div className="space-y-3">
                    {builderModules.map((mod) => (
                      <div key={mod.id} className="p-4 border border-[var(--border)] rounded-xl bg-[var(--bg-primary)]/40 flex items-center justify-between gap-4">
                        <div>
                          <p className="text-xs font-bold text-blue-400">Module {mod.id} : {mod.titleFr}</p>
                          <p className="text-[10px] text-[var(--text-muted)] mt-1">{mod.lessons.length} Lesson(s) ready with coding exercises</p>
                        </div>
                        <button
                          type="button"
                          onClick={() => handleRemoveModule(mod.id)}
                          className="p-1.5 hover:bg-rose-500/10 text-rose-500 rounded"
                        >
                ...
                <button
                  ...
                  className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase tracking-wider rounded-lg flex items-center justify-center gap-1.5"
                >
              ...
              <>
                ...
                {coursesList.filter((c) => !enrolledCourseIds.includes(c.id)).length > 0 && (
                  <section className="space-y-4">
